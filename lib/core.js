...
function startup(arg) {
    ...
    if (config.debug && !config.log_level) config.log_level = "debug";
...
function createBot() {
    ...
    bot.on("system.login.slider", () => {
        process.stdin.once("data", (input) => {
            bot.sliderLogin(ticketPick(input));
        });
    });
...
function createServer() {
    ...
    server = http.createServer((req, res) => {
        ...
        if (httpOptions(req, res) === "end") {
            return;
        }
        if (config.access_token) {
            if (!req.headers["authorization"]) {
                const access_token = querystring.parse(url.parse(req.url).query)
                    .access_token;
    ...
    if (config.use_ws) {
        ...
        wss.on("connection", (ws, req) => {
            ws.on("error", () => {});
            if (config.access_token) {
                if (!req.headers["authorization"]) return ws.close(1401);
                if (!req.headers["authorization"].includes(config.access_token))
    ...
    server
        .listen(config.port, config.host, () => {
            bot.logger.info(
                `开启http服务器成功，监听${server.address().address}:${
                    server.address().port
                }`
            );
        })
...
function createWSClient(url, headers) {
    try {
        ...
        ws.on("close", () => {
            bot.logger.error(
                `反向ws连接(${url})被关闭，将在${config.ws_reverse_reconnect_interval}毫秒后尝试连接。`
            );
...
function debug(msg) {
    if (bot && bot.logger) bot.logger.debug(msg);
